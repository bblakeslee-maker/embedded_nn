function [netScores] = lenetSynthTensor(inputImg, weightsConv1, biasConv1, weightsConv2, biasConv2, weightsFC1, biasFC1, weightsFC2, biasFC2, weightsFC3, biasFC3)
...
%   inputImg: 32x32x1 MNIST image
    ...
    for f = 1:1:6
        ...
        for r = 1:1:28
            for c = 1:1:28
                ...
                conv1ActivationMap(r, c, :) = sum(sum(filter .* single(imgRegion))) + biasConv1(f);
            end
        end
    end
    ...
    % Layer 9: fc2
    fc2ActivationMap = relu3ActivationMap' * weightsFC2 + biasFC2;
    
    % Layer 10: fc3
    netScores = fc2ActivationMap * weightsFC3 + biasFC3;
